...
public class Script_ManualFall : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        //マニュアルとゲームで同じこれを使っているので１度ゲームしてスローか時止めで終わったときに
        //正しく動くようにタイムスケールをリセットしておく
        Script_GameManager.globalTimeScale = 1.0f;
    ...
    private IEnumerator Move(GameObject obj, Vector3 dir, float speed)
    {
        ...
        while (obj != null)
        {
            ...
            obj.transform.position += dir * speed * Time.deltaTime * Script_GameManager.globalTimeScale;
